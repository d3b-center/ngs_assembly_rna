...
# Updated merge step from Komal Rathi's code in OpenPBTA
# Date: 05/10/2020
# Function:
...
option_list <- list(
  ...
  make_option(c("-c", "--old_rsem_count"),
    type = "character",
    help = "Path to old merged count files", default = NULL
  ),
...
if (mergefiles) {
  ...
  if (!is.null(old_rsem_count)) {
    ...
    # save output
    saveRDS(all_rnaseq_rsem.counts, file = file.path(outdir, paste0(outname_prefix, "-gene-counts-rsem-expected_count.rnaseq.rds")))
  }
  ...
  if (!is.null(old_rsem_fpkm)) {
    ...
    # save output
    saveRDS(all_rnaseq_rsem.fpkm, file = file.path(outdir, paste0(outname_prefix, "-gene-expression-rsem-fpkm.rnaseq.rds")))
  }
  ...
  if (!is.null(old_rsem_tpm)) {
    ...
    # save output
    saveRDS(all_rnaseq_rsem.tpm, file = file.path(outdir, paste0(outname_prefix, "-gene-expression-rsem-tpm.rnaseq.rds")))
  }
