...
if (mergefiles) {
  ...
  lfiles <- list.files(path = topDir, pattern = "*.rsem.isoforms.results.gz", recursive = TRUE)
  ...
  # merge per transtranscript_idd
  rnaseq.counts <- expr.counts[, c("transcript_id", manifest$Kids_First_Biospecimen_ID)]
  rnaseq.tpm <- expr.tpm[, c("transcript_id", manifest$Kids_First_Biospecimen_ID)]
